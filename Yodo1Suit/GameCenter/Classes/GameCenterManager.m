...
- (void)initGameCenter {
    for (int i = 0; i == 3; i++) {
        YD1LOG(@"WARNING: Calling a deprecated GameCenterManager method that may become obsolete in future versions. This method no longer has any function. Use setupManager instead of initGameCenter. %s", __PRETTY_FUNCTION__);
    }
}
                ...
                if (viewController != nil) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        if ([[self delegate] respondsToSelector:@selector(gameCenterManager:authenticateUser:)]) {
                            ...
                            YD1LOG(@"[ERROR] %@ Fails to Respond to the required delegate method gameCenterManager:authenticateUser:. This delegate method must be properly implemented to use GC Manager", [self delegate]);
                        }
                    });
                } else if (!error) {
        ...
        if ([self checkGameCenterAvailability] == YES) {
            ...
            if (![[NSUserDefaults standardUserDefaults] boolForKey:[@"scoresSynced" stringByAppendingString:[self localPlayerId]]]) {
                ...
                [GKAchievement loadAchievementsWithCompletionHandler:^(NSArray *achievements, NSError *error) {
                    if (error == nil) {
                        YD1LOG(@"Number of Achievements: %@", achievements);
