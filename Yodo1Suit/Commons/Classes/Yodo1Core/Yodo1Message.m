...
//  Joyhus
//
//  Created by hyx on 2019/8/20.
//  Copyright © 2019 hyx. All rights reserved.
//

#import "Yodo1Message.h"
#import "Yodo1Base.h"
...
+ (void)cc_appDelegateMethod:(SEL)selector params:(id)param,... {
    ...
    for (NSString *name in keys) {
        ...
        NSInteger paramsCount = signature.numberOfArguments - 2;
        NSMutableArray *paramsList = [Yodo1Base.shared cc_init:NSMutableArray.class];
        va_list params;
        va_start(params, param);
        int i = 0;
        for (id tmpObject = param; 1; tmpObject = va_arg(params, id)) {
            if (tmpObject) {
                ...
                [paramsList addObject:[Yodo1Base.shared cc_init:NSNull.class]];
            }
            i++;
            if (i >= paramsCount) {
                break;
...
+ (id)cc_targetAppDelegate:(NSString *)appDelegateName method:(NSString *)method block:(void (^)(BOOL success))block params:(id)param,... {
    SEL selector = NSSelectorFromString(method);
    id appDelegate = Yodo1CoreBase.shared.sharedAppDelegate[appDelegateName];
    ...
    if (target && [target respondsToSelector:selector]) {
        ...
        for (NSInteger i = 0; i < paramList.count; i++) {
            ...
            if (![obj isKindOfClass:NSNull.class]) {
                ...
                if ([obj isKindOfClass:Yodo1Int.class]) {
                    Yodo1Int *ccint = (Yodo1Int *)obj;
                    int value = ccint.value;
                    [invocation setArgument:&value atIndex:i + 2];
                } else if ([obj isKindOfClass:Yodo1Float.class]) {
                    Yodo1Float *ccfloat = (Yodo1Float *)obj;
                    float value = ccfloat.value;
                    [invocation setArgument:&value atIndex:i + 2];
                } else if ([obj isKindOfClass:Yodo1Double.class]) {
                    Yodo1Double *ccdouble = (Yodo1Double *)obj;
                    double value = ccdouble.value;
                    [invocation setArgument:&value atIndex:i + 2];
                } else if ([obj isKindOfClass:Yodo1NSUInteger.class]) {
                    Yodo1NSUInteger *ccnsuinterge = (Yodo1NSUInteger *)obj;
