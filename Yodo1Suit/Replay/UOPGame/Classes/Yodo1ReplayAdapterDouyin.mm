...
@interface Yodo1ReplayAdapterDouyin()<UOPReplayManagerMetalDataSource,UOPReplayManagerDelegate>
...
- (void)initWithConfig:(Yodo1ReplayConfig* _Nonnull)replayConfig delegate:(id<Yodo1ReplayManagerDelegate> __nullable)delegate {
    ...
    config.unionMode = UOPGameUnionModeLive;
...
- (void)setType:(Yodo1ReplayType)type {
    ...
    } else if (self.replayType == Yodo1ReplayTypeManual) {
    ...
    [UOPReplayManager sharedManager].metalDataSource = self;
...
#pragma mark UOPReplayManagerMetalDataSource
// 如果游戏渲染接口为Metal，则本方法【必须】实现！
// 如果游戏渲染接口为OpenGL，则本方法【不要】实现！
