...
@implementation NSArray (Yodo1MASAdditions)

- (NSArray *)mas_makeConstraints:(void(^)(Yodo1MASConstraintMaker *make))block {
...
- (NSArray *)mas_updateConstraints:(void(^)(Yodo1MASConstraintMaker *make))block {
    NSMutableArray *constraints = [NSMutableArray array];
    for (MAS_VIEW *view in self) {
        NSAssert([view isKindOfClass:[MAS_VIEW class]], @"All objects in the array must be views");
...
- (void)mas_distributeViewsAlongAxis:(MASAxisType)axisType withFixedSpacing:(CGFloat)fixedSpacing leadSpacing:(CGFloat)leadSpacing tailSpacing:(CGFloat)tailSpacing {
    ...
    if (axisType == MASAxisTypeHorizontal) {
        ...
        for (int i = 0; i < self.count; i++) {
            MAS_VIEW *v = self[i];
            [v mas_makeConstraints:^(Yodo1MASConstraintMaker *make) {
                if (prev) {
