...
#import "Yodo1FMResultSet.h"
#import "Yodo1FMDatabasePool.h"
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `Yodo1FMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<Yodo1FMResultSet>` - Represents the results of executing a query on an `Yodo1FMDatabase`.
 - `<Yodo1FMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<Yodo1FMDatabasePool>` - A pool of `Yodo1FMDatabase` objects.
 - `<Yodo1FMStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `Yodo1FMDatabase` object and use it across multiple threads. Instead, use `<Yodo1FMDatabaseQueue>`.
...
@interface Yodo1FMDatabase : NSObject
...
/** Create a `Yodo1FMDatabase` object.
 
 An `Yodo1FMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `Yodo1FMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `Yodo1FMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    Yodo1FMDatabase *db = [Yodo1FMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    Yodo1FMDatabase *db     = [Yodo1FMDatabase databaseWithPath:dbPath];
 ...
 @return `Yodo1FMDatabase` object if successful; `nil` if failure.
...
/** Create a `Yodo1FMDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `Yodo1FMDatabase` connection is closed.
...
/** Initialize a `Yodo1FMDatabase` object.
...
/** Initialize a `Yodo1FMDatabase` object.
 ...
 An `Yodo1FMDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
/** Execute select statement
 ...
 Executing queries returns an `<Yodo1FMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[Yodo1FMResultSet next]>`) from one record to the other.
 ...
 @return A `<Yodo1FMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see Yodo1FMResultSet
 @see [`Yodo1FMResultSet next`](<[Yodo1FMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (Yodo1FMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (Yodo1FMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (Yodo1FMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> Yodo1FMResultSet!`
...
- (Yodo1FMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (Yodo1FMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (Yodo1FMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    Yodo1FMResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [Yodo1FMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using Yodo1FMDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `Yodo1FMStatement`, but rather with `<Yodo1FMDatabase>` and `<Yodo1FMResultSet>` only.
...
@interface Yodo1FMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
