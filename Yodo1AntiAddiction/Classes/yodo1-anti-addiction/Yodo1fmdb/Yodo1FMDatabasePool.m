...
- (void)pushDatabaseBackInPool:(Yodo1FMDatabase*)db;
- (Yodo1FMDatabase*)db;

@end


@implementation Yodo1FMDatabasePool
...
- (void)pushDatabaseBackInPool:(Yodo1FMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The Yodo1FMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
