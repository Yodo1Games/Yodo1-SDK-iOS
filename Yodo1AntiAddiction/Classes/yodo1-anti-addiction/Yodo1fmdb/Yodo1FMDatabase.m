...
- (Yodo1FMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray * _Nullable)arrayArgs orDictionary:(NSDictionary * _Nullable)dictionaryArgs orVAList:(va_list)args;
...
@implementation Yodo1FMDatabase
...
#pragma mark Yodo1FMDatabase instantiation and deallocation
...
// if ([Yodo1FMDatabase FMDBVersion] >= 0x0240) { … }
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    Yodo1FMDatabase *self = (__bridge Yodo1FMDatabase*)f;
...
// but for folks who don't bother noticing that the interface to Yodo1FMDatabase changed,
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        Yodo1FMResultSet *rs = (Yodo1FMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(Yodo1FMResultSet *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (Yodo1FMStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (Yodo1FMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(Yodo1FMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(Yodo1FMStatement*)statement forQuery:(NSString*)query {
    NSParameterAssert(query);
    if (!query) {
        NSLog(@"API misuse, -[Yodo1FMDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
- (BOOL)goodConnection {
    ...
    Yodo1FMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The Yodo1FMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The Yodo1FMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The Yodo1FMDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The Yodo1FMDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"Yodo1FMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (Yodo1FMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}

- (Yodo1FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    Yodo1FMStatement *statement  = 0x00;
    Yodo1FMResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[Yodo1FMStatement alloc] init];
    ...
    rs = [Yodo1FMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (Yodo1FMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (Yodo1FMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (Yodo1FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (Yodo1FMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    Yodo1FMResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (Yodo1FMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    Yodo1FMStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[Yodo1FMStatement alloc] init];
...
int Yodo1FMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int Yodo1FMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) FMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? Yodo1FMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(BOOL *rollback))block {
    ...
    NSString *errorMessage = NSLocalizedStringFromTable(@"Save point functions require SQLite 3.7", @"FMDB", nil);
    if (self.logsErrors) NSLog(@"%@", errorMessage);
    return [NSError errorWithDomain:@"Yodo1FMDatabase" code:0 userInfo:@{NSLocalizedDescriptionKey : errorMessage}];
#endif
}
...
void Yodo1FMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void Yodo1FMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (void*)b, &Yodo1FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (__bridge void*)b, &Yodo1FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
@implementation Yodo1FMStatement
